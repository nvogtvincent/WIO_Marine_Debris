...
def release_loc_land(param, fh):
    ...
    if param['plot_input'] == 'all':
        ...
        f = plt.figure(figsize=(25, 28), constrained_layout=True)
        gs = GridSpec(2, 3, figure=f, width_ratios=[1, 0.02, 0.02])
        ax = []
        ...
        ax.append(f.add_subplot(gs[:, 2])) # Colorbar for plastics
        ...
        # Plot currents + wind
        winter_uo = (ref_o_uo[-1, :, ]+ref_o_uo[0, :, ]+ref_o_uo[1, :, ])/3
        summer_uo = (ref_o_uo[5, :, ]+ref_o_uo[6, :, ]+ref_o_uo[7, :, ])/3
        winter_vo = (ref_o_vo[-1, :, ]+ref_o_vo[0, :, ]+ref_o_vo[1, :, ])/3
        summer_vo = (ref_o_vo[5, :, ]+ref_o_vo[6, :, ]+ref_o_vo[7, :, ])/3

        winter_u10 = (ref_w_u10[-1, :, ]+ref_w_u10[0, :, ]+ref_w_u10[1, :, ])/3
        summer_u10 = (ref_w_u10[5, :, ]+ref_w_u10[6, :, ]+ref_w_u10[7, :, ])/3
        winter_v10 = (ref_w_v10[-1, :, ]+ref_w_v10[0, :, ]+ref_w_v10[1, :, ])/3
        ...
        cplot.append(ax[0].pcolormesh(ref_o_lon, ref_o_lat, winter_o_vel, cmap=cmo.tempo,
        ...
        ax[0].quiver(ref_w_lon[::subset_ref], ref_w_lat[::subset_ref],
                      ...
                      winter_v10[::subset_ref, ::subset_ref], color='k',
        ...
        ax[0].text(80, 38, 'Northeast Monsoon (DJF)', fontsize=48, color='k', zorder=20,
                   fontweight='bold', ha='center', va='top')
        ax[0].text(80, 33, 'and direct coastal plastic input', fontsize=34, color='k', zorder=20,
                   fontweight='bold', ha='center', va='top')

        cplot.append(ax[1].pcolormesh(ref_o_lon, ref_o_lat, summer_o_vel, cmap=cmo.tempo,
                                      vmin=0, vmax=1.0, transform=ccrs.PlateCarree(),
                                      rasterized=True))

        ax[1].quiver(ref_w_lon[::subset_ref], ref_w_lat[::subset_ref],
                     summer_u10[::subset_ref, ::subset_ref],
                     summer_v10[::subset_ref, ::subset_ref], color='k',
                     scale=2e2, scale_units='height', width=2e-3, headwidth=4,
                     headlength=4)
        ...
        ax[1].text(80, 38, 'Southwest monsoon (JJA)', fontsize=48, color='k', zorder=20,
                   fontweight='bold', ha='center', va='top')
        ax[1].text(80, 33, 'and riverine plastic input', fontsize=34, color='k', zorder=20,
                   fontweight='bold', ha='center', va='top')
        ...
        cb1 = plt.colorbar(ScalarMappable(norm=norm, cmap=cmr.torch),
                           cax=ax[3], orientation='vertical')
        cb1.set_label('Annual plastic flux (tonnes)', size=42)
        ax[3].tick_params(axis='y', labelsize=36)

        plt.savefig(fh['fig'] + '.pdf', dpi=300, bbox_inches='tight')
...
def release_loc_land_uniform(param, fh):
    # Generates initial coordinates for particle releases at a single time
    # frame based on the ISO codes for countries of interest.
    ...
    with Dataset(fh['grid'], mode='r') as nc:
        ...
        lon_rho = np.array(nc.variables['lon_rho'][:])
        id_psi  = np.array(nc.variables['iso_psi'][:])

    # Now find the cells matching the provided ISO codes
    id_psi[~np.isin(id_psi, ids)] = 0

    # Now distribute plastic flux per grid cell across particles
    # Apply plastic threshold
    print('')
    print('Total cells with plastic: ' + str(np.count_nonzero(id_psi)))

    # Extract cell grid indices
    idx = list(np.where(id_psi > 0)) # Contains grid indices of all grid cells with plastic above the threshold

    # Calculate the total number of particles
    nl  = idx[0].shape[0]  # Number of locations
    pn_tot = nl*param['pn']**2
    ...
    lon_out = np.zeros((pn_tot,), dtype=np.float64)
    lat_out = np.zeros((pn_tot,), dtype=np.float64)
    id_out = np.zeros((pn_tot,), dtype=np.int16)
    ...
    for loc in range(nl):
        ...
        # Calculate initial positions
        dx = dX/param['pn']                    # Particle spacing
        gx = np.linspace((-dX/2 + dx/2), (dX/2 - dx/2), num=param['pn'])
        gridx, gridy = [grid.flatten() for grid in np.meshgrid(gx, gx)]
        ...
        s_idx = loc*param['pn']**2
        e_idx = (loc+1)*param['pn']**2

        lon_out[s_idx:e_idx] = gridx + loc_x
        lat_out[s_idx:e_idx] = gridy + loc_y
        id_out[s_idx:e_idx] = np.ones(np.shape(gridx), dtype=np.int16)*loc_id
...
def add_times(particles, param):
    ...
    try:
        tot_parts = param['total_partitions']
    except:
        tot_parts = 1

    if tot_parts > 1:
        pn_per_part = int(np.ceil(npart/tot_parts))
