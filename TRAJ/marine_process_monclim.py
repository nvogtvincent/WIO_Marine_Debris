        ...
        try:
            with xr.open_dataset(fh, mask_and_scale=False) as file:
                ...
                if n_traj:
                    ...
                    frame['sink_month'] = frame['sink_date_'].dt.month
                    frame['days_at_sea'] = frame['days_at_sea'].dt.days
                    frame['source_month'] = m0

                    # Clean up
                    frame.drop(labels=['sink_date_'], axis=1, inplace=True)
...
month_bnds = np.arange(0.5, 13.5)

# Flux matrix dimensions:
# LAT | LON | TIME
fmatrix = np.zeros((len(lat), len(lon), 12, 12), dtype=np.float32)
    ...
    for month in np.arange(12):
        for release in np.arange(4):
            ...
            fmatrix += np.histogramdd(np.array([data[data['sink_iso'].isin(param['sites'])]['lat0'],
                                                ...
                                                data[data['sink_iso'].isin(param['sites'])]['source_month'],
                                                data[data['sink_iso'].isin(param['sites'])]['sink_month']]).T,
