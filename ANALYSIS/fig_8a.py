...
corr = xs.pearson_r(fmatrix_lp, clim, dim=time_var)
corr_p_eff = xs.pearson_r_eff_p_value(fmatrix_lp, clim, dim=time_var)

##############################################################################
# PLOT                                                                       #
##############################################################################

f = plt.figure(figsize=(19.75, 11.5), constrained_layout=True)
...
sig_plot1 = ax[0].contourf(fmatrix_lp.coords['longitude'], fmatrix_lp.coords['latitude'], corr_p_eff.where((corr_p_eff > 0.01)*(corr_p_eff <= 0.05)).T,
                            levels=np.array([0.01, 0.05]), hatches=['/'], colors='none')
sig_plot2 = ax[0].contourf(fmatrix_lp.coords['longitude'], fmatrix_lp.coords['latitude'], corr_p_eff.where(corr_p_eff <= 0.01).T,
...
title = param['mode'] + ' correlation at Aldabra Group'
