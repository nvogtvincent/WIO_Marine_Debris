...
for class_name in param['name']:
    corr.append(xr.DataArray(np.zeros((n_site, n_lead), dtype=np.float32),
                             coords={'site': param['site_list'],
                                     'lead': lead_idx},
                             name=class_name))
    pval.append(xr.DataArray(np.zeros((n_site, n_lead), dtype=np.float32),
                             coords={'site': param['site_list'],
                                     'lead': lead_idx},
                             name=class_name))
...
n_month = 240
i0_fh = dirs['data'] + '/terrestrial_flux_' + param['physics'][0] + '_s' + str(param['us_d'][0]) + '_b' + str(param['ub_d'][0]) + '.nc'
...
for i, class_name in enumerate(param['name']):
    md_fh = dirs['data'] + '/terrestrial_flux_' + param['physics'][i] + '_s' + str(param['us_d'][i]) + '_b' + str(param['ub_d'][i]) + '.nc'
    ...
    for site in param['site_list']:
        ...
        corr[i].loc[site, :] = xs.pearson_r(beaching_ts_lp, clim_idx, dim='sink_time')
        pval[i].loc[site, :] = xs.pearson_r_eff_p_value(beaching_ts_lp, clim_idx, dim='sink_time')
...
for i, class_name in enumerate(param['name']):
    corr_vis1.append(ax[i].pcolormesh(np.append(lead_idx, lead_idx[-1]+1)-0.5, np.arange(n_site+1), corr[i].where(pval[i] > 0.01),
                                      cmap=cm.prinsenvlag_r, vmin=-1, vmax=1))
    corr_vis2.append(ax[i].pcolormesh(np.append(lead_idx, lead_idx[-1]+1)-0.5, np.arange(n_site+1), corr[i].where(pval[i] <= 0.01),
                                      cmap=cm.fusion_r, vmin=-1, vmax=1))
    ax[i].set_aspect(1)
...
cb2.set_label('Correlation', size=20)
ax[4].tick_params(axis='y', labelsize=17)
